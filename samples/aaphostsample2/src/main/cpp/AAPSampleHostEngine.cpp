...
int32_t AAPHostEngineBase::onAudioReady(void *audioData, int32_t numFrames) {
    if (state != AAP_HOST_ENGINE_STATE_ACTIVE)
...
void AAPHostEngineBase::initialize(aap::PluginClientConnectionList* connections, int32_t sampleRate, int32_t audioOutChannelCount, int32_t aapFrameSize) {
    assert(connections);
...
void AAPHostEngineBase::terminate() {
    ...
    aap::a_log_f(AAP_LOG_LEVEL_INFO, LOG_TAG, "Successfully terminated host engine.");
}

std::string AAPHostEngineBase::convertStateToText(AAPHostEngineState stateValue) {
    switch (stateValue) {
        case AAP_HOST_ENGINE_STATE_CREATED:
            return "CREATED";
        case AAP_HOST_ENGINE_STATE_ACTIVE:
            return "ACTIVE";
        case AAP_HOST_ENGINE_STATE_INACTIVE:
            return "INACTIVE";
        case AAP_HOST_ENGINE_STATE_STOPPED:
            return "STOPPED";
        case AAP_HOST_ENGINE_STATE_ERROR:
            return "ERROR";
    }
    return "(UNKNOWN)";
}
...
void AAPHostEngineBase::instantiatePlugin(std::string pluginId) {
    ...
    if (instance_data) {
        ...
        if (instance)
            aap::a_log_f(AAP_LOG_LEVEL_ERROR, LOG_TAG, "There is an already instantiated plugin \"%s\" for this HostEngine.",
    ...
    client->createInstanceAsync(pluginId, sample_rate, true, [&](int32_t instanceId, std::string error) {
        ...
        state = AAP_HOST_ENGINE_STATE_INACTIVE;

        aap::a_log_f(AAP_LOG_LEVEL_INFO, LOG_TAG, "instantiated plugin %s",
                     pluginId.c_str());
    });
}

// Activate audio processing. Starts audio (oboe) streaming, CPU-intensive operations happen from here.
void AAPHostEngineBase::activate() {
    if (state != AAP_HOST_ENGINE_STATE_INACTIVE) {
    ...
    state = AAP_HOST_ENGINE_STATE_ACTIVE;
}

// Deactivate audio processing. CPU-intensive operations stop here.
void AAPHostEngineBase::deactivate() {
    if (state != AAP_HOST_ENGINE_STATE_ACTIVE) {
...
void AAPHostEngineBase::callPluginProcess() {
...
void AAPHostEngineBase::fillAudioOutput() {
...
void* AAPHostEngineBase::getAAPMidiInputBuffer() {
    auto data = instance_data.get();
    assert(data);
...
// this function.
void AAPHostEngineBase::processMidiInput(uint8_t* bytes, size_t offset, size_t length, int64_t timestampInNanoseconds) {
