...
int runHostLilv(int sampleRate, const char **pluginUris, int numPluginUris, void *wav, int wavLength, void *outWav) {
    ...
    for (int i = 0; i < numPluginUris; i++) {
        ...
        if (plugin == NULL)
            ...
            LilvInstance *instance = lilv_plugin_instantiate(plugin, sampleRate,
                                                             features);
            if (instance == NULL) {
...
jint Java_org_androidaudiopluginframework_hosting_AAPLV2LocalHost_runHostLilv(JNIEnv *env, jclass cls, jobjectArray jPlugins, jint sampleRate, jbyteArray wav, jbyteArray outWav)
{
    jboolean isCopy = JNI_TRUE;
    ...
    int ret = aaplv2::runHostLilv(sampleRate, pluginUris, size, wavBytes, wavLength, outWavBytes);

    env->SetByteArrayRegion(outWav, 0, wavLength, (jbyte*) outWavBytes);

    aaplv2::set_io_context(NULL);
